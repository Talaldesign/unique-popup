...
function renderSocials(socials){
  ...
  socials.forEach(s=>{
    ...
    img.addEventListener('click', ()=>{
      ...
      supabaseClient.from('analytics').insert([{ event_type:'social_click', info: JSON.stringify({name:s.name,url:s.url}), visitor_id: localStorage.getItem('visitorId') }]).catch(()=>{});
    });
...
async function initPage(){
  ...
  supabaseClient.from('analytics').insert([{ event_type:'visit', visitor_id: localStorage.getItem('visitorId') }]).catch(()=>{});
